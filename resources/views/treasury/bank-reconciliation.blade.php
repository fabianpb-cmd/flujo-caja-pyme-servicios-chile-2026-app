...
@if ($statementSummary)
<div class="alert alert-secondary app-panel">Cobertura de cartola: {{ $statementSummary['matched'] }} matched · {{ $statementSummary['unmatched_system'] }} movimientos sistema sin matching · {{ $statementSummary['unmatched_bank'] }} líneas banco sin matching · {{ $statementSummary['ignored'] }} ignoradas. <a href="{{ route('bank-statements.index', ['cash_account_id' => $selected?->id]) }}">Gestionar cartolas bancarias</a>. Esto no altera la regla de cierre con diferencia cero.</div>
@endif
...
<div class="app-panel section-card"><div class="section-card-header"><h2 class="section-card-title">Movimientos posted de la cuenta</h2></div><div class="table-responsive"><table class="table align-middle"><thead><tr><th>Fecha</th><th>Código</th><th>Referencia</th><th>Contraparte</th><th class="text-end">Ingreso</th><th class="text-end">Egreso</th><th>Cartola</th></tr></thead><tbody>@forelse ($movements as $movement)<tr><td>{{ \App\Support\UiFormatter::formatDate($movement->movement_date) }}</td><td>{{ $movement->code }}</td><td>{{ $movement->reference ?: '—' }}</td><td>{{ $movement->counterparty_name ?: '—' }}</td><td class="text-end">{{ \App\Support\UiFormatter::formatMoney($movement->income) }}</td><td class="text-end">{{ \App\Support\UiFormatter::formatMoney($movement->expense) }}</td><td>@if ($movement->activeBankStatementMatch)<span class="badge text-bg-success">Matched</span><div class="small text-muted">{{ $movement->activeBankStatementMatch->bankStatementLine?->reference ?: 'Cartola' }}</div>@else<span class="text-muted">Sin matching</span>@endif</td></tr>@empty<tr><td colspan="7" class="text-center text-muted">Sin movimientos posted para la cuenta.</td></tr>@endforelse</tbody></table></div></div>
@endsection
