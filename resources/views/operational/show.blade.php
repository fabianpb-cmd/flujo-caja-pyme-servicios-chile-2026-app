        ...
        <div class="app-panel p-3 mb-4">
            ...
            @foreach ($billingPlan['milestones'] as $row)
                ...
                <div class="border rounded p-2 mb-2">
                    <div class="d-flex flex-wrap justify-content-between gap-2"><div><strong>{{ $milestone->sequence }}. {{ $milestone->name }}</strong><div class="small text-muted">{{ \App\Support\UiFormatter::formatPercent($milestone->percentage / 100, 2) }} · {{ \App\Support\UiFormatter::formatMoney($row['amount'], $billingCurrency) }} · Fecha prevista: {{ $milestone->planned_invoice_date ? \App\Support\UiFormatter::formatDate($milestone->planned_invoice_date) : 'Sin fecha prevista' }}</div></div><div>@if($row['invoiced']) <span class="badge text-bg-success">Facturado</span> @else <form method="POST" action="{{ route('projects.milestones.issue', [$item, $milestone]) }}" class="d-inline">@csrf<input class="form-control form-control-sm d-inline-block" style="width: 10rem" type="date" name="issue_date" value="{{ now()->toDateString() }}" required><input type="hidden" name="taxable" value="1"><button class="btn btn-sm btn-success">Facturar</button></form> @endif</div></div>
                </div>
            @endforeach
        </div>
    @endif
