    ...
    @if ($resource === 'time-entries' && (! $editing || $isTimeEntryBatchEdit))
        ...
        <div data-time-entry-period-panel data-time-entry-period-load-container>
            ...
            <div class="row g-3 mb-3">
                ...
                <div class="col-12 col-md-6 col-xl-3" data-time-entry-period-total-hours-wrap>
...
@else
        @if ($resource === 'projects')
            ...
            @php($projectContractCode = strtoupper((string) data_get($projectSelectedContractData, 'code')))
            @php($projectIsClosed = $projectContractCode === 'PROYECTO_CERRADO' || mb_strtolower((string) data_get($projectSelectedContractData, 'label')) === 'proyecto cerrado')
            @php($projectIsHoursBank = $projectContractCode === 'BOLSA_HORAS')
            @php($projectIsMonthlyRecurring = $projectContractCode === 'MENSUAL_RECURRENTE')
            ...
            @php($projectMilestoneTotal = $projectBillingPlanReadOnly ? (float) $item->billingMilestones->sum('percentage') : collect($projectMilestoneRows)->sum(fn ($row) => is_numeric($row['percentage'] ?? null) ? (float) $row['percentage'] : 0))
            <div class="app-panel p-3 mb-4" data-project-billing-plan data-closed="{{ $projectIsClosed ? '1' : '0' }}">
                ...
                <div data-project-billing-hourly class="small text-muted {{ $projectIsClosed || $projectIsHoursBank || $projectIsMonthlyRecurring ? 'd-none' : '' }}">Modalidad: Facturación por HH aprobadas · Periodicidad actual: Mensual · Base: HH aprobadas no facturadas del período.<br>Sin bolsa contractual. La tarifa cobrada al cliente por cada HH aprobada se configura como Tarifa comercial HH.</div>
                <div data-project-billing-hours-bank class="small text-muted {{ $projectIsHoursBank ? '' : 'd-none' }}">Modalidad: Bolsa total consumible. Se factura por HH aprobadas y la bolsa no se reinicia mensualmente.<br>Valor bolsa: {{ \App\Support\UiFormatter::formatMoney($item->sale_net ?? 0, $item->salesCurrency ?: 'CLP') }} · Tarifa HH: {{ \App\Support\UiFormatter::formatMoney($item->contracted_hourly_rate ?? 0, $item->salesCurrency ?: 'CLP') }}@if(($projectHoursCapacity['capacity_hours'] ?? null) !== null) · Capacidad: {{ \App\Support\UiFormatter::formatHours($projectHoursCapacity['capacity_hours']) }} · Consumidas: {{ \App\Support\UiFormatter::formatHours($projectHoursCapacity['consumed_hours']) }} · Disponibles: {{ \App\Support\UiFormatter::formatHours($projectHoursCapacity['remaining_hours']) }}@endif</div>
                <div data-project-billing-monthly class="small text-muted {{ $projectIsMonthlyRecurring ? '' : 'd-none' }}">Modalidad: Bolsa mensual consumible. Se factura por HH aprobadas del período; la bolsa se reinicia cada mes y no acumula saldo.<br>Valor mensual contratado: {{ \App\Support\UiFormatter::formatMoney($item->sale_net ?? 0, $item->salesCurrency ?: 'CLP') }} · Tarifa HH: {{ \App\Support\UiFormatter::formatMoney($item->contracted_hourly_rate ?? 0, $item->salesCurrency ?: 'CLP') }}@if(($projectHoursCapacity['capacity_hours'] ?? null) !== null) · Capacidad mensual: {{ \App\Support\UiFormatter::formatHours($projectHoursCapacity['capacity_hours']) }} · Consumo del mes actual: {{ \App\Support\UiFormatter::formatHours($projectHoursCapacity['consumed_hours']) }} · Disponible: {{ \App\Support\UiFormatter::formatHours($projectHoursCapacity['remaining_hours']) }}@endif</div>
                ...
                <div data-project-billing-milestones class="{{ $projectIsClosed ? '' : 'd-none' }}">
                    ...
                    <template data-project-billing-milestone-template>
                        <div class="row g-2 mb-2" data-project-milestone-row>
                            <div class="col-1"><input required class="form-control" name="billing_milestones[0][sequence]" type="number" min="1" value="" placeholder="#"></div>
                            <div class="col-3"><input required class="form-control" name="billing_milestones[0][name]" value="" placeholder="Nombre del hito"></div>
                            <div class="col-3"><input class="form-control" name="billing_milestones[0][planned_invoice_date]" type="date" value=""></div>
                            <div class="col-2"><input required class="form-control" name="billing_milestones[0][percentage]" type="number" min="0.01" max="100" step="0.01" value="" placeholder="%"></div>
                            <div class="col-3 d-flex gap-2"><input class="form-control" name="billing_milestones[0][notes]" value="" placeholder="Notas"><button type="button" class="btn btn-outline-danger" data-project-billing-remove aria-label="Eliminar hito">Eliminar</button></div>
                        </div>
                    </template>
            ...
            @push('scripts')
                <script nonce="{{ $cspNonce ?? '' }}">
                    (() => { const panel = document.querySelector('[data-project-billing-plan]'); const select = document.getElementById('contract_type_id'); if (!panel || !select) return; const milestones = panel.querySelector('[data-project-billing-milestones]'); const form = panel.closest('form'); const submit = form?.querySelector('button[type="submit"]'); const total = panel.querySelector('[data-project-billing-total]'); const remaining = panel.querySelector('[data-project-billing-remaining]'); const warning = panel.querySelector('[data-project-billing-warning]'); const saleNetLabel = form?.querySelector('label[for="sale_net"] span'); const format = value => Number(value).toLocaleString('es-CL', { maximumFractionDigits: 2, useGrouping: false }); const syncPercentages = () => { const percentageInputs = milestones?.querySelectorAll('input[name$="[percentage]"]') || []; if (percentageInputs.length === 0) return; const sum = [...percentageInputs].reduce((total, input) => total + (Number.parseFloat(input.value) || 0), 0); const rounded = Math.round(sum * 100) / 100; const excess = Math.max(0, rounded - 100); if (total) total.textContent = format(rounded); if (remaining) remaining.textContent = format(Math.max(0, 100 - rounded)); if (warning) { warning.textContent = excess > 0 ? `La suma de hitos supera el 100% en ${format(excess)} %.` : ''; warning.classList.toggle('d-none', excess <= 0); } if (submit) submit.disabled = excess > 0; }; const sync = () => { const option = select.options[select.selectedIndex]; const code = (option?.dataset?.code || '').toUpperCase(); const label = (option?.textContent || '').trim().toLowerCase(); const closed = code === 'PROYECTO_CERRADO' || label === 'proyecto cerrado'; const hoursBank = code === 'BOLSA_HORAS' || ['bolsa de horas', 'bolsa horas'].includes(label); const monthly = code === 'MENSUAL_RECURRENTE' || label === 'mensual recurrente'; const supportedHourly = ['POR_HORA', 'POR_HORAS', 'HOURLY'].includes(code) || ['por hora', 'por horas'].includes(label); panel.dataset.closed = closed ? '1' : '0'; const rate = form?.querySelector('#contracted_hourly_rate'); if (rate) rate.readOnly = closed; if (saleNetLabel) saleNetLabel.textContent = monthly ? 'Valor mensual contratado' : 'Venta neta'; milestones?.classList.toggle('d-none', !closed); milestones?.querySelectorAll('input').forEach(input => input.disabled = !closed); panel.querySelector('[data-project-billing-hourly]')?.classList.toggle('d-none', closed || hoursBank || monthly); panel.querySelector('[data-project-billing-hours-bank]')?.classList.toggle('d-none', !hoursBank); panel.querySelector('[data-project-billing-monthly]')?.classList.toggle('d-none', !monthly); panel.querySelector('[data-project-billing-unsupported]')?.classList.toggle('d-none', closed || hoursBank || monthly || supportedHourly); syncPercentages(); }; milestones?.addEventListener('input', syncPercentages); select.addEventListener('change', sync); sync(); })();
                </script>
                <script nonce="{{ $cspNonce ?? '' }}">
                    (() => { const panel = document.querySelector('[data-project-billing-plan]'); const milestones = panel?.querySelector('[data-project-billing-milestones]'); const add = panel?.querySelector('[data-project-billing-add]'); const template = panel?.querySelector('[data-project-billing-milestone-template]'); if (!panel || !milestones || !add || !template) return; const renumber = () => milestones.querySelectorAll('[data-project-milestone-row]').forEach((row, index) => row.querySelectorAll('[name]').forEach(input => { input.name = input.name.replace(/billing_milestones\[\d+\]/, `billing_milestones[${index}]`); })); const bindRemove = row => row.querySelector('[data-project-billing-remove]')?.addEventListener('click', () => { if (milestones.querySelectorAll('[data-project-milestone-row]').length <= 1) return; row.remove(); renumber(); milestones.dispatchEvent(new Event('input', { bubbles: true })); }); milestones.querySelectorAll('[data-project-milestone-row]').forEach(bindRemove); add.addEventListener('click', () => { const row = template.content.firstElementChild.cloneNode(true); const used = new Set([...milestones.querySelectorAll('input[name$="[sequence]"]')].map(input => Number.parseInt(input.value, 10)).filter(Number.isInteger).filter(value => value > 0)); let sequence = 1; while (used.has(sequence)) sequence += 1; row.querySelector('input[name$="[sequence]"]').value = sequence; const next = [...milestones.querySelectorAll('[data-project-milestone-row]')].find(existing => Number.parseInt(existing.querySelector('input[name$="[sequence]"]')?.value, 10) > sequence); milestones.insertBefore(row, next || template); bindRemove(row); renumber(); milestones.dispatchEvent(new Event('input', { bubbles: true })); }); })();
                </script>
            @endpush
        @endif
        @if (! ($resource === 'time-entries' && (! $editing || $isTimeEntryBatchEdit)))
        ...
        const renderTimeEntryPeriodPreview = (preview = null) => {
            ...
            if (timeEntryPeriodSummaryRate) {
                timeEntryPeriodSummaryRate.textContent = hasValidPreview && summary.shared_rate_display
                    ...
                    : (summary.multiple_rates
                        ? 'Valor HH de costeo del proyecto: Variable.'
            ...
            if (timeEntryPeriodMultipleSummary) {
                ...
                if (hasValidPreview && summary.multiple_rates) {
                    notes.push('Valor HH de costeo: Variable dentro del período.');
                }
