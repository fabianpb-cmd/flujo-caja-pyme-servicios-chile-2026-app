...
final class UatHtml
{
    ...
    public static function parseIndexRows(string $html): array
    {
        ...
        foreach ($xpath->query('//table//tbody/tr') ?: [] as $row) {
            ...
            // No depender del texto visible de la acción ("Ver"). Algunas tablas
            // usan iconos, botones "Editar" o menús de acciones. Conservamos todos
            // los href de la fila para que locateOperationalRecord() pueda resolver
            // el ID desde cualquier URL del recurso.
            $links = [];
            ...
            $preferredShow = null;
            foreach ($xpath->query('.//a[normalize-space()="Ver"]', $row) ?: [] as $link) {
                if ($link instanceof DOMElement && trim($link->getAttribute('href')) !== '') {
                    $preferredShow = trim($link->getAttribute('href'));
                    break;
                }
            }

            $rows[] = [
                'show_url' => $preferredShow,
...
final class StagingMultiuserUatRunner
{
    ...
    private function bootstrap(): void
    {
        ...
        $this->recordCase('Logout/relogin', function (): array {
            ...
            $logout = $this->user1->request($logoutForm['method'], $logoutForm['action'], $logoutForm['fields']);
            $this->assertStatus($logout, 200, 'Logout usuario 1 debe completar redirect a login.');

            $afterLogout = $this->user1->get('/dashboard', [], false);
            if (! in_array($afterLogout->status, [302, 303], true)) {
                throw new StagingUatException('Usuario 1 debería perder la sesión tras logout.');
            }

            $user2StillAlive = $this->user2->get('/dashboard');
            $this->assertStatus($user2StillAlive, 200, 'Usuario 2 debe mantener su sesión tras logout usuario 1.');

            $relogin = $this->login($this->user1, (string) $this->env['UAT_USER1_EMAIL'], (string) $this->env['UAT_USER1_PASSWORD']);
            $this->assertStatus($relogin, 200, 'Relogin usuario 1 debe responder 200.');

            return ['status' => 'PASS', 'evidence' => 'Logout aislado + relogin exitoso'];
        });
    }
    ...
    private function createExpenseDocument(array $client, array $project, string $suffix, float $grossTarget, float $vatRate): array
    {
        return $this->recordCreate('Gasto '.$suffix, function () use ($client, $project, $suffix, $grossTarget, $vatRate): array {
            ...
            $record = $this->createOperationalRecord('expense-documents', [
                ...
                'expense_subcategory_id' => ['first_non_empty' => true],
    ...
    private function createOperationalRecord(string $resource, array $overrides, ?string $searchTerm): array
    {
        ...
        foreach ($overrides as $field => $value) {
            $payload[$field] = $this->resolveFieldValue($field, $value, $form['options'] ?? []);
        }

        $response = $this->user1->request($form['method'], $form['action'], $payload);
        if ($response->status >= 500) {
            throw new StagingUatException("Alta {$resource} devolvió {$response->status}.");
        }

        if (str_contains($response->body, 'alert-danger')) {
            $errors = implode(' | ', UatHtml::extractTextBySelector($response->body, '//div[contains(@class,"alert-danger")]//li|//div[contains(@class,"alert-danger")]'));
            throw new StagingUatException("Alta {$resource} con error de validación: {$errors}");
        }

        $located = $this->locateOperationalRecord($resource, $searchTerm);
        $show = $this->user1->get('/operacion/'.$resource.'/'.$located['id']);
        $this->assertStatus($show, 200, "Show {$resource} debe responder 200.");
        $details = UatHtml::parseDefinitionList($show->body);

        return $this->normalizeRecord($resource, $located['id'], $details, $located['show_url']);
    }
    ...
    private function locateOperationalRecord(string $resource, ?string $searchTerm): array
    {
        ...
        if ($rows === []) {
            throw new StagingUatException("No se encontraron filas en {$resource} tras la creación.");
        }

        $row = null;
        if ($searchTerm !== null) {
            foreach ($rows as $candidate) {
                if (str_contains($candidate['text'], $searchTerm)) {
                    $row = $candidate;
                    break;
                }
            }
        ...
        if (! empty($row['show_url'])) {
            $candidateUrls[] = (string) $row['show_url'];
        }
        ...
        $id = null;
        foreach (array_values(array_unique($candidateUrls)) as $candidateUrl) {
            $path = parse_url($candidateUrl, PHP_URL_PATH);
            $path = is_string($path) ? $path : $candidateUrl;
        ...
        if ($id === null) {
            throw new StagingUatException(
                "No se pudo identificar el ID de {$resource} en las acciones de la fila encontrada."
            );
        }

        // Usar siempre la URL canónica de detalle; así el runner no depende de
        // si la fila expone Ver, Editar o un botón con icono.
    ...
    private function resolveFieldValue(string $field, mixed $value, array $options): string
    {
        if (! is_array($value)) {
            return (string) $value;
        }
        ...
        return (string) ($fieldOptions[0]['value'] ?? '');
    }
    ...
    {
        $numeric = preg_replace('/\D+/', '', $seed) ?: '12345678';
        $number = (int) substr(str_pad($numeric, 8, '1'), 0, 8);
        $number = max(1000000, ($number + $offset) % 99999999);
