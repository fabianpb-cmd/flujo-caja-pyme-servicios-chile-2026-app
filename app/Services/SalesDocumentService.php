...
class SalesDocumentService
{
    ...
    public function confirm(SalesDocument $document, ?\App\Models\User $user = null, ?string $documentNumber = null): SalesDocument
    {
        return DB::transaction(function () use ($document, $user, $documentNumber): SalesDocument {
            ...
            $number = $documentNumber === null ? trim((string) $locked->document_number) : trim($documentNumber);
            if ($number === '') throw new DomainException('La factura requiere número de documento.');
            if (blank($locked->issue_date)) throw new DomainException('La factura requiere fecha de emisión.');
            $before = $locked->toArray();
            $locked->forceFill(['document_number' => $number, 'status' => 'Pendiente'])->save();
