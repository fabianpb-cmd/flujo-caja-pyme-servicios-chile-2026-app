...
| Pago por hora y multiplicidad de contexto | En remuneración por hora, la base económica se calcula como tarifa HH de Personal × `TimeEntry.hours_approved` consumidas. La multiplicidad de proyectos o asignaciones utilizadas no altera la tarifa de Personal ni vuelve inválido el cálculo. Si todos los `TimeEntry` consumidos pertenecen a un único proyecto, ese proyecto puede persistirse en `payroll_records.project_id`; si pertenecen a varios proyectos, `project_id` puede quedar `null` y la trazabilidad detallada permanece en `payroll_record_time_entries`. |
