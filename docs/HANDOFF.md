...
- Conciliación Bancaria V1.0: **CERRADA / PASS** en producción.
- Regularización Bancaria V1.1: **CERRADA / PASS** en producción.
- Cartolas bancarias + matching V1.2: **CERRADA / PASS** en producción.
- Menú productivo: **CERRADO / PASS**.
- Cuentas por cobrar: semántica productiva validada.
- Cuentas por pagar: semántica productiva validada.
- Breadcrumbs, active states y sidebar responsive: **PASS**.
- Regresión V1 / V1.1 / V1.2: **PASS**.
...
- Esto no constituye una afirmación de cero bugs.
- Existe deuda/test histórico date-sensitive de `FinancialAgendaTest`.
- Existen fallos históricos o de entorno previamente documentados, incluido `ZipArchive` local, que no corresponden a regresiones nuevas.

### QA productiva relevante

Fixtures QA que deben mantenerse documentadas:

- `CTA-000007`: QA conciliación histórica.
- `CTA-000008`: QA conciliación post-cutover.
- `CTA-000009`: QA V1.1 Post Overlay.
- `CTA-000010`: QA V1.2 Statement Matching.
- `MOV-000012` / `MOV-000013`: QA histórico V1.1.
- `MOV-000014`: QA conciliación.
...
El smoke productivo final validó import CSV con BOM y `;`, dedupe, sugerencia con score 90, matching, reversión, rematch, ignore, inmutabilidad de `CashMovement`, saldo esperado/observado `1.012.345`, Cash Flow sin doble conteo, conciliación cerrada y bloqueo correcto de la reversión posterior.

### Menú

Orden productivo final de Tesorería:

1. Cuentas
2. Movimientos de caja
3. Cartolas bancarias
4. Regularización bancaria
5. Conciliación bancaria

CxC y CxP ya no son aliases semánticos de los listados generales:
...
FACTURACIÓN GUIADA POR FUENTE CONTRACTUAL V1 - VALIDADA LOCALMENTE / PUSH Y QA PENDIENTES - 2026-09-13

La creación de Facturas/Ingresos ahora presenta un flujo guiado por la estrategia contractual real del proyecto. Para `CLOSED_PROJECT` muestra exclusivamente hitos pendientes y genera el borrador mediante `ProjectBillingMilestoneService::issue`; para `HOURLY` usa exclusivamente la prefacturación de HH aprobadas mediante `SalesPrefacturationService::generateDraft`; sin proyecto o con estrategia no soportada conserva la creación manual existente. La fecha prevista del hito se presenta solo como referencia y no genera emisión automática.

La vista previa de hitos es server-side y reutiliza el servicio de hitos para monto contractual, conversión a CLP, IVA, vencimiento y proyección de cobro. El endpoint y la emisión validan empresa, pertenencia del hito al proyecto y la factura activa del hito. El CRUD genérico rechaza por servidor intentos manuales de crear Facturas/Ingresos para proyectos `CLOSED_PROJECT` o `HOURLY`; la protección no depende de JavaScript. La emisión conserva los flujos existentes: documentos automáticos quedan en Borrador, no consumen ni duplican HH y no crean movimientos de caja.

Cobertura añadida: `GuidedSalesBillingTest` 3 tests / 41 assertions PASS. Cubre el proyecto cerrado con hito pendiente, vista previa, borrador, reemisión y bloqueo de monto manual; el aislamiento proyecto/empresa del endpoint de hitos; y el proyecto Por Hora con prefacturación, vínculo HH, prevención de duplicidad y bloqueo de POST manual, además de la creación manual sin proyecto.

Regresiones dirigidas PASS: `ProjectBillingMilestoneServiceTest` 12/92, `SalesPrefacturationTest` 12/58, `SalesDocumentConfirmationTest` 11/42, `ProfitabilityServiceTest` 8/32, `ProjectFinancialCockpitTest` 4/31, `FinancialNavigationTest` 3/22 y `SecurityHardeningTest` 6/85. Total ejecutado: 59 tests / 403 assertions PASS. `php artisan view:cache` y `git diff --check`: PASS.

Archivos relevantes: `OperationalCrudController`, `ProjectBillingMilestoneController`, `ProjectBillingMilestoneService`, `routes/web.php`, `resources/views/operational/sales-guided-form.blade.php` y `tests/Feature/GuidedSalesBillingTest.php`. Sin migraciones, sin SQL, sin deploy y sin acceso a producción. Pendiente: commit, push y QA manual del flujo guiado.

AYUDANTE TDAT V1 - VALIDADO LOCALMENTE / PUSH Y QA MANUAL PENDIENTES - 2026-09-13

Se incorporó un panel contextual read-only disponible en pantallas autenticadas. La ruta propia `POST /ayudante/preguntar` usa `auth`, `absolute.session`, `admin.2fa`, `sensitive.no-store` y CSRF. El navegador nunca se comunica con OpenAI: envía solo pregunta, metadatos de pantalla, estado vacío/informado de campos e IDs permitidos. El panel conserva la conversación únicamente en el DOM actual y escapa toda respuesta como texto.

Arquitectura: `AssistantController` -> `AssistantOrchestrator` -> `AssistantContextService`, `AssistantKnowledgeService`, `AssistantBusinessContextService`, `AssistantResponseGuard` y contrato `AiProvider` con implementación `OpenAiResponsesProvider` para Responses API. El proveedor está aislado por configuración; no hay SDK nuevo, no hay acceso del modelo a SQL/Eloquent arbitrario y no se persisten preguntas, respuestas, prompts ni conversaciones.

La base auditable `config/assistant_knowledge.php` cubre reglas globales, clientes, proyectos, horas, facturación, CxC, gastos, tesorería, cartolas, conciliación, remuneraciones y gestión. El retrieval incluye reglas globales y un máximo de 12 reglas relevantes. Toda respuesta pertenece a `DEFINED`, `CALCULATED`, `REQUIRED`, `NOT_DEFINED` o `ADMIN_REQUIRED`; la guardia degrada a `NOT_DEFINED` las fuentes inventadas, respuestas sin fuente verificable y afirmaciones de operaciones mutativas.

Contexto y privacidad: los recursos se validan exclusivamente contra `config/operational.php`; el contexto de negocio resuelve Proyecto/Hito solo con `forCompany` y reutiliza preview read-only de hitos. Se excluyen password, 2FA, recovery codes, tokens, RUT, correo, teléfonos, direcciones, cuentas bancarias, números de documento, notas y campos desconocidos. Solo los IDs explícitos `client_id`, `project_id`, `milestone_id`, `cash_account_id` y la fecha de emisión segura se consideran para contexto controlado.

Provider y costos: feature flag `AI_ASSISTANT_ENABLED=false` por defecto; si está apagado responde controladamente sin invocar proveedor. Variables seguras agregadas a `.env.example`; la API key no se versiona. Límites por usuario configurables: 10/minuto y 200/día. Errores timeout, autenticación, rate limit o JSON inválido retornan mensajes seguros sin cuerpo del proveedor, prompt ni credenciales.
...
CORRECCIÓN RESPONSES API OUTPUT - VALIDADA LOCALMENTE - 2026-09-14
...
AYUDANTE TDAT - GUÍA DE UTILIZACIÓN CONTEXTUAL - VALIDADO LOCALMENTE - 2026-09-14

El Ayudante TDAT ahora recibe `FORM_GUIDE` read-only para cada recurso operativo conocido. La guía se deriva dinámicamente de `config/operational.php`; no replica un manual de campos ni envía valores de registros. Incluye pantalla, secciones, metadatos seguros de campos, clasificación, requeridos, opcionales, campos calculados, dependencias, validaciones relevantes y campos requeridos actualmente vacíos solo cuando `FORM_CONTEXT` lo demuestra.

`AssistantScreenGuideService` clasifica código generado, campos calculados/readonly, selección de catálogo, dependencias condicionales, opcionales y entradas de usuario. Para relations/select explica únicamente que se debe seleccionar un valor existente; no sugiere opciones ni completa valores. La guía conserva estados empty/filled, no valores personales, financieros ni sensibles.

En Proyectos, la guía resuelve `contract_type_id` solo dentro de la empresa del usuario y adapta el contexto a Por Hora, Bolsa de horas, Mensual recurrente o Proyecto cerrado. La semántica contractual permanece en `config/assistant_knowledge.php` y `docs/domain-contract.md`: Bolsa total no reinicia; Mensual recurrente reinicia por mes, sin carryover, prorrateo ni overage automático.

El orquestador entrega `KNOWLEDGE_RULES`, `FORM_CONTEXT`, `FORM_GUIDE` y `BUSINESS_CONTEXT` al provider. `FORM-GUIDE:<resource>` es una fuente controlada aceptada por la guardia; el prompt prohíbe usarla para inventar valores o semántica. La guardia también rechaza respuestas que indiquen montos o fechas concretas no respaldadas. Se agregó flujo confirmado Cliente → Proyecto → Asignación → Horas → Facturación, y sus transiciones verificadas hacia CxC/CxP, caja, cartolas y conciliación.
...
Cobertura focalizada PASS: `ProjectBillingPlanHttpTest` 14 tests / 83 assertions y `ConsumableContractBillingTest` 9 tests / 43 assertions. Cubre metadata de códigos para Proyecto cerrado, Bolsa de horas y Mensual recurrente; las vistas Por Hora, Bolsa total, Mensual recurrente y Proyecto cerrado; y el fallback server-side por nombre de catálogo. `php artisan view:cache` y `git diff --check`: PASS. Sin migraciones, sin SQL ni deploy.
